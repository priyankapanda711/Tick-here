...
payment_button?.addEventListener('click',function () {
    ...
    formdata.append('seats',seats as any);
...
if (event_Id) {
  ...
  fetch(apiUrl)
    ...
    .then((data) => {
      ...
      (document.getElementById("event-title") as HTMLElement).textContent = event.title;
      (document.getElementById("event-duration") as HTMLElement).textContent = formatDuration(event.duration);
      (document.getElementById("event-description") as HTMLElement).textContent = event.description;
      (document.getElementById("event_category") as HTMLElement).textContent = event.event_category.name;
      (document.getElementById("event-price") as HTMLElement).textContent = "1299";
